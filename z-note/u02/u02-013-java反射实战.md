...
- 动态编译需要使用 `javax.tools.ToolProvider`：
    ...
    - `int run(InputStream in, OutputStream out, OutputStream err, String... arguments)`：执行javac
        - p1：inputStream输入流，传递给javac的数据，如果为null，表示使用 `System.in`
        - p2：outputStream输出流，javac返回的数据，如果为null，表示使用 `System.out`
        - p3：outputStream输出流，javac返回的错误信息，如果为null，表示使用 `System.err`
