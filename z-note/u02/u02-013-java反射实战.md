...
- 设计一个实体类User.java，其中需要有id、userName和userAge这个三个私有属性。
- 设计一个注解类@Table，整个运行期有效，只能作用在类上，参数只有一个value，必填，对应数据库表名。
- 设计一个注解类@Id，整个运行期有效，它只能作用在属性上，空注解，对应数据库的主键，主键名默认为字段名，格式默认INTEGER，大小默认11。
- 设计一个注解类@Column，整个运行期有效，只能作用在属性上，参数name对应字段名，参数type对应字段类型，参数length对应字段大小，字段名必填，类型和大小不填写默认（"varchar" 和 50）。
- 设计一个注解解析工具类CreateTableTool，其方法 `build()` 的功能是，将User的类对象作为参数，返回对应建表的SQL语句。
