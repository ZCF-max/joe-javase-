...
**测试：** SoftReferenceTest.java(-Xms20M -Xmx20M)
```java
...
**概念：** 堆内存分为，分为新生代、老年代，默认内存大小比例是1：2，这个比例值是可以通过 `-XX:NewRatio` 运行参数来动态设置的。
- 新生代 `Young Generation`：动态存储新new出来的对象，新生代又被划分为3个区域：伊甸园 `Eden` 和两个 `Survivor` 区（也叫幸存区），两个幸存区分别叫 `Survivor from` 和 `Survivor to`。
    - `Eden` 区：实例一开始都在Eden区产生，当Eden满了，JVM执行一次 `minorGC`，`minorGC` 会在整个新生代里面执行GC动作，80%-90%的实例会在这里直接死去，存活下来的幸存实例会被拷贝到某个S1区（假设为S1），且将幸存对象身上的计数器加1，Eden被清空，同时S2区中存活下来的幸存实例也会被拷贝到S1，因为JVM规定必须保证至少有一个Survivor区是空的），另外，垃圾回收过程中年龄足够老的对象（计数器值过大）直接进入老年代。整个过程采取的是复制算法，因为是从Eden将幸存者复制到S1或者S2的。
    - `Survivor` 区：达到某个条件时，会将大于Survivor区的一半相对年龄大的实例都移入老年代，另外，在某次 `minorGC` 的过程中，年龄足够老的对象（计数器值过大）也会直接进入老年代。
- 老年代 `Old Generation`：存储年龄稍大的实例，主要被 `MajorGC` 回收，`MajorGC` 的回收速度比 `minorGC` 低10倍，采用标记清除或者标记移动的算法，达到某个条件时JVM会执行一次 `FullGC`，整体大回收。
