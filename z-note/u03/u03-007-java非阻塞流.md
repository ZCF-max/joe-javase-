...
- 直接缓冲区：DirectBuffer
    ...
    - 创建方式：`XXXBuffer.ballocateDirect(1024)`：必须指定初始缓冲区容量字节大小。
- 非直接缓冲区：HeapBuffer
...
**概念：** 阻塞式IO的两端是App和硬盘数据，而非阻塞式NIO的Buffer和硬盘数据之间，通过一个Channel接口来通信。
- Channel常用实现类：
    - `FileChannel`：文件操作的通道。
...
- Channel常用API方法：以ByteBuffer为例
    - `int read(ByteBuffer dst)`：将硬盘文件数据读到Buffer中，读完返回-1。
    - `int write(ByteBuffer src)`：将Buffer中的数据写到硬盘文件中。

## 3.1 FileChannel

**概念：** FileChannel通道负责连接硬盘文件数据和Buffer。

**源码：** nio/ChannelTest.fileChannel()
