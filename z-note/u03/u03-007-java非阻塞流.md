...
- NIO模型：非阻塞式IO模型：当用户发起一个read请求后，立刻得到结果，这个结果是你要读的硬盘数据是否已经加载到了内核空间，如果已经加载到了，开始从内核空间读取数据到用户空间（这个过程是阻塞的），如果没有加载到，每隔一段时间就重问一遍（间隔轮训），这种模式下，因为每次都会立刻得到结果而无需等待，所以不会阻塞，用户可以做其他的事情。
