...
- Channel常用API方法：以ByteBuffer为例
    - `int read(ByteBuffer dst)`：将硬盘数据读到Buffer中，读完返回-1。
    - `int write(ByteBuffer src)`：将Buffer中的数据写到硬盘数据中。

## 3.1 FileChannel

**概念：** FileChannel需要通过基础IO流实例获取得到，负责连接硬盘文件数据和App中的Buffer。

**源码：** nio/FileChannelTest.java

## 3.2 SocketChannel

> 创建一个Socket地址连接需要使用 `new InetSocketAddress()` 构造，客户端需要在执行构造的同时指定一个包含了字符串IP和数字端口号的Socket地址，而服务端只需在使用之前绑定(bind)一个指定了端口号的Socket地址即可。
