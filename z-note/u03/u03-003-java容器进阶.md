...
**源码：** TreeSet的底层是二叉树结构，可以对存储的内容进行排序，线程不同步。
- TreeSet只会对实现了 `Comparable` 的接口的类进行自然排序，如String、Integer等。
- TreeSet在构造的时候，可以直接指定一个 `java.util.Comparator` 的实现类作为比较器，并重写 `compare()`，然后自己定义比较规则。
- 我们可以自定义一个类，让该类实现Comparable的接口并重写compareTo()，然后自己定义比较规则。

**源码：** collection/TreeSetTest.java
