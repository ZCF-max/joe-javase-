...
- 方法：这里主要使用LinkedList专属的方法，父接口List中的方法同ArrayList。
    ...
    - `boolean offerLast(E e)`：将e添加到链表结尾，底层调用的 `addList()`。
    - `E get(int index)`：获取index位置上的元素。
    - `E getFirst()`：获取头但是不移除头，空列表抛异常。
    - `E element()`：获取头但是不移除头，空列表抛异常，底层调用 `getFirst`。
    - `E peek()`：获取头但是不移除头，空列表返回null，1.5版本。
    - `E peekFirst()`：获取头但是不移除头，空列表返回null，1.6版本。
    - `E peekLast()`：获取尾但是不移除尾，空列表返回null，1.6版本。  
    - `E set(int index, E e)`：将index位置上的元素替换成e。
    - `E poll()`：获取头并且移除头，空列表返回null，1.5版本。
    - `E pollFirst()`：获取头并且移除头，空列表返回null，1.6版本。
    - `E removeFirst()`：获取头并且移除头，空列表抛异常。
    - `E pop()`：获取头并移除头，底层调用的 `removeFirst()`，1.5版本。
    - `E remove()`：获取头并移除头，底层调用的 `removeFirst()`，1.6版本。    
    - `E removeLast()`：获取尾并且移除尾，空列表抛异常           
    - `E pollLast()`：获取尾并且移除尾，空列表返回null，1.6版本。
