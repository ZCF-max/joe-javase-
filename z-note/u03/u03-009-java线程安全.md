...
**源码：** sync/TicketSellTest.java

# 2. synchronized

**概念：** 关键字synchronized可以将代码进行线程隔离，其本质是非公平锁，在字节码指令级，synchronized底层使用 `monitorenter` 指令表示进入同步代码区（获取锁），使用 `monitorexit` 指令表示离开同步代码区（释放锁），处于同步隔离区的代码，只能被所有线程排队执行，牺牲效率以保证数据安全。
- synchronized有两种用法：
...
- 同步部分代码：使用 `synchronized(){}` 只对涉及到共享数据的部分代码加锁，小括号中是锁的类型。
    ...
    - 锁实例建议填写final，因为一旦运行中途，锁实例被改变了，则同步效果也会立即消失。
- 加了syn之后就不用再加volatile了。
- 加锁原则：粒度尽量小，数量尽量少。

**源码：** sync/TicketSellProTest.java
...
**源码：** sync/LockTypeTest.java

## 2.2 锁的可重入性

**概念：** syn的锁具有重入性，比如m1()是加锁的，m2()是加锁的，在m1()中可以调用m2()，因为是同一个线程申请的这把锁，允许重入同步代码区。
- 如果因重入而获取了5次锁，则对应的必须要释放5次锁，所以锁的重入次数必须被记录下来，在hotspot实现中，加非OS锁时会在线程栈中生成一个LR（Lock Record），其中将锁实例的mark-word信息进行备份（主要就是实例的hashcode）因为加锁后，锁的信息会覆盖掉这些信息，然后每重入一次，则都会再次入栈一个空的LR，重入5次锁，则入栈5个空LR，每释放一次锁，弹出1个空LR，5个空LR均被弹出，则表示全部重入锁释放完毕，最后再释放最后一个带有mark-word备份信息的LR（释放前还原备份信息），完成完整的同步操作。
...
**源码：** sync/AtomicIntegerArrayDemo.java
...
源码：sync/AtomicLongFieldUpdaterDemo.java

> 操作字符串类型的name，更换使用AtomicReferenceFieldUpdater即可。

# 4. ThreadLocal的使用

**概念：** 线程变量，是一个以ThreadLocal对象为键、任意对象为值的存储结构。为每个线程单独存放一份变量副本，也就是说一个线程可以根据一个ThreadLocal对象查询到绑定在这个线程上的一个值。
