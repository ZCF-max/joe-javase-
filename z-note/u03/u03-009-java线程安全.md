...
**概念：** 关键字synchronized可以将代码进行线程隔离，其本质是非公平锁，在字节码指令级，synchronized底层使用 `monitorenter` 指令表示进入同步代码区（获取锁），使用 `monitorexit` 指令表示离开同步代码区（释放锁），处于同步隔离区的代码，只能被所有线程排队执行，牺牲效率以保证数据安全，synchronized有两种用法：
...
- 加锁原则：粒度尽量小，数量尽量少，只在拥有共享数据的地方加锁。
