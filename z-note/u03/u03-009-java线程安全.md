...
**概念：** ThreadLocal被称为本地线程，内部维护了一个以当前线程实例为key、以任意值为value的ThreadLocalMap结构，每个线程都有一个独立的ThreadLocalMap，且每个ThreadLocalMap只能存储一个值。
- 方法：
    - `void set(T value) `: 将 `value` 设置到当前线程的ThreadLocalMap中，其他线程获取不到。
    - `T get()`: 获取当前线程的ThreadLocalMap中存储的 `value`。
    - `void remove()`: 移除当前线程的ThreadLocalMap中存储的 `value`。
        - ThreadLocal使用完一定要调用 `remove`，否则会产生内存泄露问题。
