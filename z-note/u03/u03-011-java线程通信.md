...
**概念：** Condition的本质就是多个等待队列，wait-notify模型只能随机唤醒某个线程的，通过Condition可以精准的控制唤醒哪一类线程，Condition实例需要通过Lock接口调用 `newCondition()`。
- 方法：
    - `void await()`：让线程进入等待状态。
    - `void signal()`：唤醒当前Condition中，某个 `await()` 状态的线程。
    - `void signalAll()`：唤醒当前Condition中，所有 `await()` 状态的线程。
