...
- 构造器：
    ...
    - `BufferedInputStream(InputStream in, int size)`：
        - p1：InputStream的子类实例。
        - p2：缓冲区默认大小，如果不指定则默认为8192个字节，即8K。
- 常用API方法：
...
- InputStreamReader：将 `inputStream` 转成 `Reader`。
- OutputStreamReader：将 `outputStream` 转成 `Writer`。

## 3.1 InputStreamReader

**测试：** 模拟Scanner键入输出，使用readLine()方法读取控制台信息。
```java
@Test
...
> Idea中的Junit和System.in或者Scanner有冲突，请换成main方法测试。
...
**概念：** 利用继承自OutputStream的PrintStream，重新定义输出打印语句的效果。
```java
...
**测试：** 使用PrintWriter创建一个日志文件，模拟Log4j（java的日志文件）工作
```java
@Test
...
> idea中，junit和scanner冲突，测试请使用main方法。