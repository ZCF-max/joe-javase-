...
- `E peek()`：查看栈顶对象，堆栈为空抛异常
- `E pop()`：出栈：查看并删除栈顶对象，堆栈为空抛异常
- `E push(E item)`：入栈：将项目推送到此堆栈的顶部
...
概念：`java.util.Queue` 接口也是一个存储数据的集合，与List、Set同一级别，都继承Collection接口，除了基本的Collection操作外，队列还提供其他的插入、提取和检查操作
